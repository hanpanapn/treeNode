...
export default {
  ...
  methods: {
    // 添加子元素
    addChild(index) {
      if (this.treeDataPri[index].child) {
        this.treeDataPri[index].child.push({ name: new Date().valueOf() });
      } else {
        this.$set(this.treeDataPri[index], "child", [{ name: new Date().valueOf() }]);
      }
    },
    // 删除子元素
    delChild(index) {
      this.treeDataPri.splice(index, 1);
    },
  },
};
</script>
